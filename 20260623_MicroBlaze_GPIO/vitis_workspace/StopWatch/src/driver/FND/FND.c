...
void FND_DispDigit(uint32_t num, uint32_t fndDP)
{
	...
	uint32_t fndData;
	if (fndDP) {
		fndData = fndFont[num % 10] & ~(0x80); //seg dp on
	} else {
		fndData = fndFont[num % 10] | (0x80); // seg dp off
	}
