...
int main()
{
    ...
    StopWatch_Init();
    int ret = SetupInterruptSystem();  // ← 여기
    xil_printf("SetupInterruptSystem ret = %d\n", ret);
    ...
    while (1) {
        // 스위치 모드 읽기
        uint8_t sw0 = GPIO_ReadPin(GPIOE, GPIO_PIN_0);
        uint8_t sw1 = GPIO_ReadPin(GPIOE, GPIO_PIN_1);
        comm_mode_e mode = (comm_mode_e)((sw1 << 1) | sw0);
